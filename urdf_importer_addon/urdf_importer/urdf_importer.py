...
def read_data(filepath, merge_duplicate_materials, should_check_material_name, rename_materials, apply_weld, unique_name, scale_unit):
    RobotBuilder(filepath, merge_duplicate_materials, should_check_material_name,
                 rename_materials, apply_weld, unique_name, scale_unit)
...
class URDFImporter(bpy.types.Operator, ImportHelper):
    ...
    rename_materials: bpy.props.BoolProperty(
        name="Rename materials", default=True)
    apply_weld: bpy.props.BoolProperty(
        name="Apply weld modifier", default=True)
    unique_name: bpy.props.BoolProperty(
        name="Each texture has an unique name", default=True)
    scale_unit: bpy.props.FloatProperty(
        name="Scale unit (for Unreal Engine is 0.01)", default=0.01)
    ...
    def execute(self, _):
        if self.merge_duplicate_materials == 'OP1':
            return read_data(self.filepath, self.merge_duplicate_materials, True, self.rename_materials, self.apply_weld, self.unique_name, self.scale_unit)
        elif self.merge_duplicate_materials == 'OP2':
            return read_data(self.filepath, self.merge_duplicate_materials, False, self.rename_materials, self.apply_weld, self.unique_name, self.scale_unit)
        else:
            return {'FINISHED'}
